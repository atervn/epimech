    ...
    while true
        ...
        % otherwise
            ...
            if any(strcmp(line{1}{1},systemParameters))
                data.parameterStudy(paramIdx).type = 'system';
            elseif any(strcmp(line{1}{1},cellParameters))
                data.parameterStudy(paramIdx).type = 'cell';
            elseif any(strcmp(line{1}{1},specificCellParameters))
                data.parameterStudy(paramIdx).type = 'specific';
            elseif (strcmp(data.simulationType,'pointlike') || strcmp(data.simulationType,'opto') || strcmp(data.simulationType,'stretch')) && any(strcmp(line{1}{1},substrateParameters))
                data.parameterStudy(paramIdx).type = 'substrate';
            ...
            data.parameterStudy(paramIdx).parameter = line{1}{1};
            
            % if the parameter values are given directly
            if strcmp(line{2},'=')
                data.parameterStudy(paramIdx).operation = 'equal';
                
            % if the parameter values are given as multipliers
            elseif strcmp(line{2},'*')
                data.parameterStudy(paramIdx).operation = 'multiply';
            ...
            % save the values
            data.parameterStudy(paramIdx).values = values;
