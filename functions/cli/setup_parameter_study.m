...
for i = 1:length(data.parameterStudy)
    % if the studied parameter is a system parameter
    if strcmp(data.parameterStudy(i).type,'system')
        
        % if the study operation is equal, set the parameter value from the
        % defined values
        if strcmp(data.parameterStudy(i).operation,'equal')
            app.systemParameters.(data.parameterStudy(i).parameter) = data.parameterStudy(i).values(iLoop);
            
        % if the study operation is multiply, multiply the selected parameter
        % with the defined multiplier
        elseif strcmp(data.parameterStudy(i).operation,'multiply')
            app.systemParameters.(data.parameterStudy(i).parameter) = app.systemParameters.(data.parameterStudy(i).parameter)*data.parameterStudy(i).values(iLoop);
        end
        
        % get the new value for the parameter
        value = app.systemParameters.(data.parameterStudy(i).parameter);
        
        % check that the new parameter fulfills the requirements for the
        % parameter in question, and if not, give error and return
        switch data.parameterStudy(i).parameter
            case 'eta'
                if isnan(value)
    ...
    elseif strcmp(data.parameterStudy(i).type,'cell')
        
        % if the study operation is equal, set the parameter value from the
        % defined values
        if strcmp(data.parameterStudy(i).operation,'equal')
            app.cellParameters.(data.parameterStudy(i).parameter) = data.parameterStudy(i).values(iLoop);
            
        % if the study operation is multiply, multiply the selected parameter
        % with the defined multiplier
        elseif strcmp(data.parameterStudy(i).operation,'multiply')
            app.cellParameters.(data.parameterStudy(i).parameter) = app.cellParameters.(data.parameterStudy(i).parameter)*data.parameterStudy(i).values(iLoop);
        end
        
        % get the new value for the parameter
        value = app.cellParameters.(data.parameterStudy(i).parameter);
    ...
    elseif strcmp(data.parameterStudy(i).type,'specific')
        
        % if the study operation is equal, set the parameter value from the
        % defined values
        if strcmp(data.parameterStudy(i).operation,'equal')
            app.specificCellParameters.(data.parameterStudy(i).parameter) = data.parameterStudy(i).values(iLoop);
            
        % if the study operation is multiply, multiply the selected parameter
        % with the defined multiplier
        elseif strcmp(data.parameterStudy(i).operation,'multiply')
            app.specificCellParameters.(data.parameterStudy(i).parameter) = app.cellParameters.(data.parameterStudy(i).parameter)*data.parameterStudy(i).values(iLoop);
        end
        
        % get the new value for the parameter
        value = app.specificCellParameters.(data.parameterStudy(i).parameter);
    ...
    elseif strcmp(data.parameterStudy(i).type,'substrate')
        
        % if the study operation is equal, set the parameter value from the
        % defined values
        if strcmp(data.parameterStudy(i).operation,'equal')
            app.substrateParameters.(data.parameterStudy(i).parameter) = data.parameterStudy(i).values(iLoop);
            
        % if the study operation is multiply, multiply the selected parameter
        % with the defined multiplier
        elseif strcmp(data.parameterStudy(i).operation,'multiply')
            app.substrateParameters.(data.parameterStudy(i).parameter) = app.substrateParameters.(data.parameterStudy(i).parameter)*data.parameterStudy(i).values(iLoop);
        end
        
        % get the new value for the parameter
        value = app.substrateParameters.(data.parameterStudy(i).parameter);
